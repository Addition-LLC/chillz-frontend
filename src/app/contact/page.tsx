      ...
      <div className="min-h-screen bg-white pt-28 pb-12 lg:pt-32">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            {/* Main Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 bg-white p-8 rounded-none shadow-lg border border-gray-100">
              
              {/* Column 1: Contact Information */}
              <div className="space-y-8">
                <h2 className="text-2xl font-bold text-black" style={{ fontFamily: 'var(--font-caviar-dreams)' }}>
                  Get in Touch
                </h2>
                ...
                <div className="space-y-6">
                  <a href="mailto:info@nhimwenluxuryhair.com" className="flex items-center gap-4 group">
                    <div className="bg-gray-100 p-3 rounded-none group-hover:bg-black group-hover:text-white transition-colors">
                      <Mail className="h-5 w-5 text-black transition-colors group-hover:text-white" />
                    </div>
                    <span className="text-black/80 group-hover:text-black transition-colors font-medium">info@nhimwenluxuryhair.com</span>
                  </a>
                  <a href="tel:+15559876543" className="flex items-center gap-4 group">
                    <div className="bg-gray-100 p-3 rounded-none group-hover:bg-black group-hover:text-white transition-colors">
                      <Phone className="h-5 w-5 text-black transition-colors group-hover:text-white" />
                    </div>
                    <span className="text-black/80 group-hover:text-black transition-colors font-medium">+1 (555) 987-6543</span>
                  </a>
                  <div className="flex items-start gap-4">
                    <div className="bg-gray-100 p-3 rounded-none mt-1">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-black text-white font-bold py-3 px-4 rounded-none hover:bg-gray-800 transition-colors duration-300 disabled:opacity-50 mt-4"
                >
...
const InputField = ({ id, name, label, error, ...props }: InputFieldProps) => (
  <div>
    <label htmlFor={id} className="block text-sm font-bold text-black mb-2" style={{ fontFamily: 'var(--font-caviar-dreams)' }}>{label}</label>
    <input 
      id={id} 
      name={name} 
      className={`block w-full rounded-none border-gray-200 bg-gray-50 shadow-sm focus:border-black focus:ring focus:ring-black/5 focus:bg-white transition-all duration-200 p-3 ${error ? 'border-red-500' : ''}`} 
      {...props} 
    />
