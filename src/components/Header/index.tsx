  ...
  return (
    <header
      className={`header top-0 left-0 z-40 flex w-full items-center transition-all duration-300 ${
        isSticky ? "fixed shadow-md bg-white" : "absolute bg-transparent"
      }`}
    >
      <div className={`container mx-auto transition-colors`}>
        <div className="relative flex items-center justify-between">
          ...
          <div className="hidden lg:flex flex-grow items-center justify-center">
            <nav className="flex space-x-8">
              ...
              <div className="group relative">
                ...
                <div
                  className="absolute top-full left-1/2 -translate-x-1/2 mt-0 p-4 shadow-lg rounded-none w-48 invisible opacity-0 translate-y-2 group-hover:visible group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 bg-white text-black"
                  style={{ fontFamily: 'var(--font-caviar-dreams)' }}
                >
                  <ul className="space-y-2">
                    {collections.map((col) => (
                      <li key={col.id}>
                        <Link href={col.path} className={`block px-2 py-1 text-sm rounded-none hover:bg-gray-100 ${ currentPath === col.path ? "font-bold" : ""}`}>
                          {col.title}
                        </Link>
                      </li>
                    ))}
                ...
                <AnimatePresence>
                 {isUserMenuOpen && (
                   <motion.div
                     ...
                     className="absolute top-full right-0 mt-1 w-48 rounded-none shadow-lg py-1 ring-1 ring-black ring-opacity-5 focus:outline-none bg-white text-black"
                   >
                     {customer ? (
                       <>
                         <Link href="/account" className="block px-4 py-2 text-sm hover:bg-gray-100" onClick={() => setIsUserMenuOpen(false)}>
                           My Account
                         </Link>
                         <button
                       ...
                       <>
                         <Link href="/login" className="block px-4 py-2 text-sm hover:bg-gray-100" onClick={() => setIsUserMenuOpen(false)}>
                           Log In
                         </Link>
                         <Link href="/signup" className="block px-4 py-2 text-sm hover:bg-gray-100" onClick={() => setIsUserMenuOpen(false)}>
                           Sign Up
                         </Link>
                       </>
                     )}
          ...
          <div className="lg:hidden">
            <button onClick={navbarToggleHandler} aria-label="Mobile Menu" className="block px-4 text-black">
      ...
      {/* Mobile Menu Panel */}
      <div className={`absolute top-full left-0 w-full bg-white text-black py-4 transition-all duration-300 lg:hidden ${navbarOpen ? "block shadow-xl" : "hidden"}`}>
        <nav className="container mx-auto">
          <ul className="flex flex-col space-y-4 px-4">
             <li><Link href="/" onClick={() => setNavbarOpen(false)} className="block text-center text-lg font-medium hover:text-gray-600">Home</Link></li>
             <li className="pt-2"><span className="block text-center text-sm font-semibold uppercase text-gray-500">Collections</span></li>
             {collections.map((col) => (
               <li key={col.id}>
                 <Link href={col.path} onClick={() => setNavbarOpen(false)} className="block text-center text-lg font-medium hover:text-gray-600">
                   {col.title}
                 </Link>
               </li>
             ))}
             <li className="pt-2"><span className="block text-center text-sm font-semibold uppercase text-gray-500">More</span></li>
              {staticLinks.map((link) => (
                 <li key={link.id}>
                   <Link href={link.path} onClick={() => setNavbarOpen(false)} className="block text-center text-lg font-medium hover:text-gray-600">
                     {link.title}
                   </Link>
                 </li>
              ))}
               ...
               <li className="pt-4 border-t border-gray-200 mt-4">
                {customer ? (
                  <>
                     <Link href="/account" onClick={() => setNavbarOpen(false)} className="block text-center text-lg font-medium hover:text-gray-600">My Account</Link>
                     <button onClick={() => { handleLogout(); setNavbarOpen(false); }} className="block w-full text-center text-lg font-medium text-red-700 mt-2 hover:text-red-800">Log Out</button>
                  </>
                ) : (
                  <>
                    <Link href="/login" onClick={() => setNavbarOpen(false)} className="block text-center text-lg font-medium hover:text-gray-600">Log In</Link>
                    <Link href="/signup" onClick={() => setNavbarOpen(false)} className="block text-center text-lg font-medium mt-2 hover:text-gray-600">Sign Up</Link>
                  </>
                )}
               </li>
          </ul>
