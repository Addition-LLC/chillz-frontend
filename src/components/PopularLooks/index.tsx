        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {products.map((product) => (
            <div key={product.id} className="group relative text-center transition-all duration-300 hover:-translate-y-2">
              <div className="relative h-[400px] w-full overflow-hidden rounded-none shadow-sm bg-white">
                <Image
                  src={product.img}
                  alt={product.name}
                  fill
                ...
                <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <Link href={product.href} className="flex items-center gap-2 rounded-none bg-white/80 px-4 py-2 text-sm font-semibold text-black backdrop-blur-sm hover:bg-white">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 z-20 bg-white/70 rounded-none p-2 backdrop-blur-sm transition-colors hover:bg-white"
                  aria-label="Toggle Favorite"
                >
                  <Heart className={`w-5 h-5 transition-all ${favorites.includes(product.id) ? 'text-red-500 fill-current' : 'text-black/70'}`} />
                </button>
              </div>
              
              <div className="mt-4">
                <h3 className="text-lg font-bold text-black" style={{fontFamily: 'var(--font-playfair-display)'}}>
                ...
                <p className="mt-1 text-md font-semibold text-black">${product.price}</p>
              </div>
            </div>
          ))}
        </div>
